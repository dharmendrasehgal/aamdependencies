...
(function($, window, document, undefined) {
	...
	jRange.prototype = {
		...
		init: function(node, options) {
            ...
            this.maxBar           = $('.max-bar', this.domNode);
			if(this.options.maxLimitTheme)
				this.maxBar.addClass(this.options.maxLimitTheme);
			this.clickableBar  = this.domNode.find('.clickable-dummy');
			this.interval      = this.options.to - this.options.from;
            this.allowedInterval      = this.options.allowedTo - this.options.from;
			this.render();
		},
		render: function() {
			...
			this.setAllowedLimit((this.options.maxLimit?this.options.maxLimit:0));
        ...
        setAllowedLimit: function(maxLimit, isPx, animate) {
			...
			var leftPos,
				...
				circleWidth = this.highPointer.width() / 2;
			if (!isPx) {
				maxLimit = this.prcToPx(maxLimit);
			}
			highPos = Math.round(maxLimit - circleWidth);
			leftPos = 0;
			this.maxBar[animate ? 'animate' : 'css']({
				'width': Math.round(highPos + circleWidth - leftPos),
				'left': leftPos
			});
		},
